...
export function Sidebar({ userRole }: SidebarProps) {
  ...
  const consultaItem = NAV_ITEMS.find((item) => item.href === "/busqueda");
  const ConsultaIcon = consultaItem ? iconMap[consultaItem.icon] : Search;
      ...
      <div className="flex flex-col mt-1 min-h-0 flex-1">
        <div className="px-3 shrink-0">
          <div className="flex items-center">
            ...
            >
              {ConsultaIcon && <ConsultaIcon className="h-4 w-4" />}
              Consulta IA
            </Link>
            <button
              onClick={() => navigate("/busqueda/nueva")}
          ...
          {groups.map((group) => (
            <div key={group.label} className="mt-2 first:mt-0">
              ...
              {group.items.map((conv) => (
                <button
                  key={conv.id}
                  onClick={() => navigate(`/busqueda/${conv.id}`)}
                  className={cn(
